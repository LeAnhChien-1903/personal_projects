...
allocation_model_folder = 'data/model/task_allocation/30x20'
...
server = RDSServer(env, max_task_gen, num_task_in_queue, num_priority, num_type, min_load, max_load, 
                            num_zone_in_cols, num_zone_in_rows, allocation_model_folder, planning_model_folder)

num_of_iteration = 100
task_text: Text = map_visual.text(20, 22.5, "", color='red')
best_time = float('inf')
...
for iter in tqdm(range(num_of_iteration)):
    ...
    if iter % 5 == 0:
        ...
        task_text_test: Text = map_visual_test.text(20, 22.5, "", color='red')
        env_test = Environment(data_folder, num_of_robot= num_of_robot, robot_max_speed= 1.0, robot_max_payload= 200, figure= figure_test, map_visual= map_visual_test)
        server_test = RDSServer(env_test, max_task_gen, num_task_in_queue, num_priority, num_type, min_load, max_load, 
